...
-- 1.1 Connect as SYSDBA and create MANAGER
...
-- 1.2 Fix privilege issues for MANAGER (granting privileges to others)
GRANT GRANT ANY PRIVILEGE TO MANAGER;
GRANT GRANT ANY OBJECT PRIVILEGE TO MANAGER;

-- 1.3 Allow MANAGER to create database-level triggers (for user creation logging)
GRANT ADMINISTER DATABASE TRIGGER TO MANAGER;

-- 1.4 Connect as MANAGER and create USER1 + USER2
conn MANAGER/"123"
...
-- 1.5 Connect as USER1 and create tables
conn USER1/"u1"
...
-- 1.6 Create required sequences
CREATE SEQUENCE AUDITTRAIL_SEQ START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE WARNINGS_SEQ START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE DBUSERCREATIONLOG_SEQ START WITH 1 INCREMENT BY 1;

-- 1.7 (Missing requirement) Allow MANAGER to log DB user creation into USER1 table
GRANT INSERT ON USER1.DBUserCreationLog TO MANAGER;
GRANT SELECT ON USER1.DBUSERCREATIONLOG_SEQ TO MANAGER;

-- 1.8 (Missing requirement) Create database trigger to log CREATE USER events
conn MANAGER/"123"

CREATE OR REPLACE TRIGGER TRG_LOG_CREATE_USER
AFTER CREATE ON DATABASE
BEGIN
  IF ORA_DICT_OBJ_TYPE = 'USER' THEN
    INSERT INTO USER1.DBUserCreationLog (id, username, created_by, created_at)
    VALUES (
      USER1.DBUSERCREATIONLOG_SEQ.NEXTVAL,
      ORA_DICT_OBJ_NAME,
      ORA_LOGIN_USER,
      SYSTIMESTAMP
    );
...
EXCEPTION
  WHEN OTHERS THEN
    NULL; -- Do not block CREATE USER if logging fails
END;
/

-- 1.9 Grant USER2 privileges on USER1 tables (done by MANAGER)
...
GRANT INSERT, SELECT ON USER1.EXAMS        TO USER2;
GRANT INSERT, SELECT ON USER1.EXAMRESULTS  TO USER2;

-- =====================================================================
-- 2. Exam Eligibility Validation
-- =====================================================================

-- 2.1 Insert sample data (as USER2)
conn USER2/"u2"

-- Insert Professors (DRs)
INSERT INTO USER1.PROFESSORS (id, name, department) VALUES (1, 'Dr. Ahmed Elsayed', 'DS');
INSERT INTO USER1.PROFESSORS (id, name, department) VALUES (2, 'Dr. Mohamed Attia', 'DS');
INSERT INTO USER1.PROFESSORS (id, name, department) VALUES (3, 'Dr. Amr Yasser', 'Rse');

-- Insert Courses
INSERT INTO USER1.COURSES (id, name, professor_id, credit_hours, prerequisite_course_id)
VALUES (101, 'Database Security', 2, 3, NULL);

INSERT INTO USER1.COURSES (id, name, professor_id, credit_hours, prerequisite_course_id)
VALUES (102, 'Adv Databases', 1, 3, 101);

INSERT INTO USER1.COURSES (id, name, professor_id, credit_hours, prerequisite_course_id)
VALUES (103, 'AI For Beginners', 1, 3, NULL);

-- Insert Students (Team)
INSERT INTO USER1.STUDENTS (id, name, academic_status, total_credits)
VALUES (932230126, 'Amr Yasser', 'Active', 0);

INSERT INTO USER1.STUDENTS (id, name, academic_status, total_credits)
VALUES (931230158, 'Doha Abdulrahman', 'Active', 0);

INSERT INTO USER1.STUDENTS (id, name, academic_status, total_credits)
VALUES (931230214, 'Kareem Ashraf', 'Active', 0);

INSERT INTO USER1.STUDENTS (id, name, academic_status, total_credits)
VALUES (932230105, 'Abdallah Adel', 'Active', 0);

INSERT INTO USER1.STUDENTS (id, name, academic_status, total_credits)
VALUES (931230372, 'Youssef Magdy', 'Active', 0);

COMMIT;

-- 2.2 Initial registrations (must match the student IDs above)
INSERT INTO USER1.REGISTER (id, student_id, course_id) VALUES (1, 932230126, 101);
INSERT INTO USER1.REGISTER (id, student_id, course_id) VALUES (2, 931230158, 101);
INSERT INTO USER1.REGISTER (id, student_id, course_id) VALUES (3, 931230214, 103);
INSERT INTO USER1.REGISTER (id, student_id, course_id) VALUES (4, 932230105, 101);
INSERT INTO USER1.REGISTER (id, student_id, course_id) VALUES (5, 931230372, 103);

COMMIT;

-- 2.3 Create prerequisite validation trigger (as USER1)
conn USER1/"u1"
...
-- =====================================================================
-- 3. Audit Trail for Registration (INSERT/DELETE)
-- =====================================================================

-- 3.1 Audit trigger for REGISTER inserts
...
-- 3.2 Audit trigger for REGISTER deletes
...
-- =====================================================================
-- 4. Grade Calculation Function
-- =====================================================================

-- 4.1 Create function to calculate grade and update ExamResults
...
-- =====================================================================
-- 5. Advanced Grade Management and Data Integrity
-- =====================================================================

-- 5.1 Ensure one ExamResults row per registration (already added, keep here for clarity)
-- (Constraint uq_examresults_reg was created after table creation)

-- 5.2 Prevent unauthorized updates to ExamResults.grade
CREATE OR REPLACE TRIGGER TRG_EXAMRESULTS_GRADE_PROTECT
BEFORE UPDATE OF grade ON ExamResults
FOR EACH ROW
BEGIN
  IF SYS_CONTEXT('USERENV','SESSION_USER') NOT IN ('USER1','MANAGER') THEN
    RAISE_APPLICATION_ERROR(-20050, 'Unauthorized: cannot update grade.');
  END IF;
END;
/

-- =====================================================================
-- 6. Insert ExamResults + Test Grade Function
-- =====================================================================

-- 6.1 Insert initial ExamResults rows (as USER1)
INSERT INTO examresults (id, registration_id, score, grade, status) VALUES (1, 1, 85, NULL, NULL);
INSERT INTO examresults (id, registration_id, score, grade, status) VALUES (2, 2, 92, NULL, NULL);
INSERT INTO examresults (id, registration_id, score, grade, status) VALUES (3, 3, 70, NULL, NULL);
COMMIT;

-- 6.2 Run the function and print results
SET SERVEROUTPUT ON;
...
BEGIN
  v_g := FN_CALC_GRADE(1);
  DBMS_OUTPUT.PUT_LINE('Calculated Grade for ER(1) = ' || v_g);

  v_g := FN_CALC_GRADE(2);
  DBMS_OUTPUT.PUT_LINE('Calculated Grade for ER(2) = ' || v_g);

  v_g := FN_CALC_GRADE(3);
  DBMS_OUTPUT.PUT_LINE('Calculated Grade for ER(3) = ' || v_g);
END;
/

-- =====================================================================
-- 7. Automated Warning Issuance
-- =====================================================================

-- 7.1 Procedure to issue warnings when a student fails in 2+ courses
...
-- 7.2 Warning test (make Kareem fail twice: registration_id=3 and new one)
INSERT INTO register (id, student_id, course_id) VALUES (2001, 931230214, 101);
COMMIT;
...
BEGIN
  v_g := FN_CALC_GRADE(3);
  DBMS_OUTPUT.PUT_LINE('Recalculated Grade for ER(3) = ' || v_g);
END;
/

INSERT INTO examresults (id, registration_id, score, grade, status)
VALUES (201, 2001, 50, 'F', 'Fail');
...
-- =====================================================================
-- 8. Course Performance Report (Cursor)
-- =====================================================================

-- 8.1 Procedure to generate a course performance report
...
-- =====================================================================
-- 9. Exam Schedule Management
-- =====================================================================

-- 9.1 Insert sample exams
...
-- 9.2 Print schedule for a course
SET SERVEROUTPUT ON;
...
-- =====================================================================
-- 10. Multi-Exam Grade Update with Transaction Control
-- =====================================================================

-- 10.1 Update multiple exam scores and recalculate grades in one transaction
SET SERVEROUTPUT ON;

DECLARE
  TYPE t_list IS TABLE OF NUMBER;

  v_regs   t_list := t_list(1, 2, 3);     -- registration IDs that exist in ExamResults
  v_scores t_list := t_list(95, 55, 80);  -- new scores
...
-- =====================================================================
-- 11. Student Suspension Based on Warnings
-- =====================================================================

-- 11.1 Suspend students who have 3+ warnings and log into AuditTrail
...
-- 12.1 Function to calculate GPA for a given student
...
-- 12.2 Example GPA call (Amr Yasser)
SELECT FN_CALC_GPA(932230126) AS GPA FROM dual;

-- =====================================================================
-- BONUS: Blocking Scenario + Identify Blocker/Waiting Sessions
-- =====================================================================

-- BONUS 11. Create a blocker-waiting situation (requires TWO separate sessions)
-- Session A (USER1):
     conn USER1/"u1"
     LOCK TABLE students IN EXCLUSIVE MODE;
     UPDATE students SET academic_status = 'Active' WHERE id = 932230126;
 
-- Session B (USER2):
     conn USER2/"u2"
     INSERT INTO USER1.register (id, student_id, course_id) VALUES (9999, 932230126, 103);

-- BONUS 12. Identify blocker and waiting sessions (run as SYSDBA)
   conn / as sysdba
